...
YOLO_FINETUNED_PATH = "models/yolo11n_finetuned.pt"
...
if model_choice == "YOLOv11 + ByteTrack (Base)":
    ...
    if os.path.exists(selected_yolo_path):
        st.sidebar.success("YOLO weights: fine-tuned `models/yolo11n_finetuned.pt`")
    else:
        st.sidebar.error("`models/yolo11n_finetuned.pt` not found. Train/copy your fine-tuned model first.")
else:
